...
{
    public static class BlowfishCipher
    {
        #region Boxes Initiation Data

        /// <summary>
        /// The P-box initialization data.
        /// </summary>
        private static readonly UInt32[] PboxInit =
        {
        ...
        /// </summary>
        private static readonly UInt32[] SboxInit0 =
        {
        ...
        /// </summary>
        private static readonly UInt32[] SboxInit1 =
        {
        ...
        /// </summary>
        private static readonly UInt32[] SboxInit2 =
        {
        ...
        /// </summary>
        private static readonly UInt32[] SboxInit3 =
        {
        ...
        public static Byte[] Key
        {
            ...
            set
            {
                ...
                Array.Copy(SboxInit0, 0, SBoxes[0], 0, 256);
                Array.Copy(SboxInit1, 0, SBoxes[1], 0, 256);
                Array.Copy(SboxInit2, 0, SBoxes[2], 0, 256);
                Array.Copy(SboxInit3, 0, SBoxes[3], 0, 256);

                InitializeArrays();
            }
        }

        private static readonly UInt32[] PBox;
        private static readonly UInt32[][] SBoxes;
        ...
        /// <summary>
        /// Initializes a new instance of the N.BlowfishCipher class with a specified key.
        /// </summary>
        static BlowfishCipher()
        {
            ...
            for (var i = 0; i < 4; i++)
                SBoxes[i] = new UInt32[256];
        }

        /// <summary>
        /// Initialize the S-boxes with the current cipher's key.
        /// </summary>
        private static void InitializeArrays()
        {
            ...
            for (var i = 0; i < 18; i++)
            {
                ...
                for (var j = 0; j < 4; j++)
                {
                    data = (data << 8) | (_key[keyIndex++]);

                    if (keyIndex >= _key.Length)
                        keyIndex = 0;
                }

                PBox[i] = PboxInit[i] ^ data;
            }
            ...
            for (var i = 0; i < 18; i += 2)
            {
                ...
                PBox[i] = BitConverter.ToUInt32(b, 0);
                PBox[i + 1] = BitConverter.ToUInt32(b, 4);
            }

            for (var i = 0; i < 4; i++)
                InitializeSBox(b, SBoxes[i]);
        }

        /// <summary>
        /// Initializes the specified S-box with the specified byte array.
        /// </summary>
        /// <param name="b">The byte array.</param>
        /// <param name="sBox">The S-box.</param>
        private static void InitializeSBox(Byte[] b, UInt32[] sBox)
        {
            for (var j = 0; j < 256; j += 2)
            {
                Encrypt(b);
                sBox[j] = BitConverter.ToUInt32(b, 0);
                sBox[j + 1] = BitConverter.ToUInt32(b, 4);
            }
        }
        ...
        public static void Encrypt(Byte[] data, Int32 offset, Int32 length)
        {
        ...
        public static void Decrypt(Byte[] data, Int32 offset, Int32 length)
        {
            var blocks = length >> 3;
        ...
        /// <param name="x"></param>
        /// <returns></returns>
        private static UInt32 F(UInt32 x)
        {
            var d = x & 0xff;
            x >>= 8;
            var c = x & 0xff;
